...
pub fn run() {
    tauri::Builder::default()
        .setup(|app| {
            ...
            if cfg!(debug_assertions) || cfg!(any(target_os = "android", target_os = "ios")) {
                app.handle().plugin(
...
async fn upload_attachment(options: UploadOptions) -> Result<ApiResponse, String> {
    ...
    info!("Reading file from path: {}", options.file_path);
    debug!("Upload options: {:?}", options);

    let file_content = tokio::fs::read(&options.file_path)
        .await
        .map_err(|e| {
            error!("Failed to read file at '{}': {}", options.file_path, e);
            format!("Failed to read file '{}': {}", options.file_path, e)
        })?;

    info!("File read successfully, size: {} bytes", file_content.len());
